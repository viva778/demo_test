...
function openHistory() {
    ...
    ReactAPI.createDialog({
        id: "unpackViewList",
        title: "拆包记录",//拆包记录
        url: baseUrl + "&customConditionKey=splitType&splitType=" + encodeURI("material_splitType/pack"),
        footer: [
            {
                name: "提交",
...
function dataInit() {
    ...
    dgDetail = ReactAPI.getComponentAPI("Datagrid").APIs("material_1.0.0_unpack_unpackEdit__mobile__dg1675150328648");
    ...
    //增加拆包记录入口
    $(".sup-navbar-right").html("<b>拆包记录</b>")
...
function ptInit() {
    ...
    //设置必填校验点
    datagrid.validator.required.setRequiredByCondition("material_1.0.0_unpack_unpackEdit__mobile__dg1675150328648", [{
        key: "decimalQuantity",
        type: "plain"
    }], function (rowData) {
        //选中时必填
    ...
    //打印完成时只读
    datagrid.readonly.setReadonlyByCondition("material_1.0.0_unpack_unpackEdit__mobile__dg1675150328648", "decimalQuantity", function (rowData) {
...
function onSave() {
    ...
    if (totalQuan > rfStock.getValue()[0].availiQuantity) {
        //拆包总数不能大于现存量
        ReactAPI.showMessage('w', getIntlValue("material.unpack.total_quantity_constraint"));
        return false;
    }
...
function getStockRefParam() {
    ...
    if (warehouse && warehouse.id) {
        return "isAble=true&wareId=" + warehouse.id;
    } else {
        return "isAble=true";
    }
}
...
function ptBtnPrint() {
    ...
    // var check_result = datagrid.validator.check("material_1.0.0_unpack_unpackEdit__mobile__dg1675150328648");
    // if (!check_result) {
    //     return false;
    // }
    //计算已打印数量
                        ...
                        if (printConfig) {
                            ...
                            ReactAPI.request({
                                ...
                                if (result.code == 200) {
                                    ...
                                    result.data.forEach(rtnData => {
                                        selData.forEach(rowData => {
                                            if (rowData.id == rtnData.dataKey) {
                                                ...
                                                datagrid.appendRowAttr("material_1.0.0_unpack_unpackEdit__mobile__dg1675150328648", newLine);
                                    ...
                                    //event.ReactAPI.showMessage('f', '打印服务接口调用失败'+result.message);
