...
const dgDetailName = "material_1.0.0_unpack_unpackEditdg1667443359750";
...
function onSave() {
    ...
    if (totalQuan.greaterThan(rfStock.getValue()[0].availiQuantity)) {
        //拆包总数不能大于现存量
        ReactAPI.showMessage('w', ReactAPI.international.getText("material.unpack.total_quantity_constraint"));
        return false;
    }
}
...
function autoSplitFun(spn, stock) {
    ...
    var newLineData = new Array(spn).fill({
        decimalQuantity: singleQuan,
        toBePrinted: true
    });
    //最后一条数量补齐remain  - (singleQuan * (spn - 1))
    newLineData[spn - 1] = {
        decimalQuantity: remain.sub(new Decimal(singleQuan).mul(spn - 1)).toFixed(decimal_round),
        toBePrinted: true
    };
    //进行增行
